#include "ffmpegpipewirescreencapture.h"


FFmpegPipeWireScreenCapture::FFmpegPipeWireScreenCapture()
{
...
bool FFmpegPipeWireScreenCapture::open(const VideoCaptureSettings &settings)
{
    close();
    ...
    const AVInputFormat *input =
        av_find_input_format("pipewire");

    if(!input)
    {
        qWarning() << "pipewire input not found.";
        return false;
    }
...
bool FFmpegPipeWireScreenCapture::start()
{
    if(!m_codec)
        return false;
...
void FFmpegPipeWireScreenCapture::stop()
{
    m_running = false;
}

void FFmpegPipeWireScreenCapture::close()
{
    stop();
...
void FFmpegPipeWireScreenCapture::setMasterTime(MasterTime *masterTime)
{
    m_masterTime = masterTime;
}

void FFmpegPipeWireScreenCapture::release(MediaFrame &frame)
{
    Q_UNUSED(frame)
}

bool FFmpegPipeWireScreenCapture::grab(MediaFrame &frame)
{
