...
void onDisconnect(bool success, QString notifyMess)
{
   if (success)
   {
      echo "Command: success" << Qt::endl;
      echo notifyMess << Qt::endl;
   }
   else
   {
      echo "Command: failure" << Qt::endl;
      echo "Error: " + notifyMess << Qt::endl;
      echo "Connection: closed" << Qt::endl;
   }
}
...
{
   echo "Socket error: " << errorString << Qt::endl;
}
...
{
   echo "File received: " + filename << Qt::endl;
}
...
{
   echo "File saving: " + filename << Qt::endl;
}
...
int main(int argc, char *argv[])
{
   ...
   if (argc<5)
   {
      echo "Usage scdimgclient <host> <port> <PUT> <file path to transfer> <dest file path>" << Qt::endl;           // single file tranfer: send a file to server
      echo "Usage scdimgclient <host> <port> <PUT> <folder path to transfer> <dest file path> -f " << Qt::endl;     // multiple file transfer: send a folder to server
      echo "Usage scdimgclient <host> <port> <GET> <remote file path to get> [-file:<file path>] [-T]" << Qt::endl; // get a file and save to disk. -T optin download a thumbnail
      echo "Usage scdimgclient <host> <port> <DEL> <remote file path to delete>" << Qt::endl;                       // delete a file from server
      return 0;
   }
   ...
   if (action=="PUT")
   {
      if (argc<6)
      {
         echo "<dest file path> required for PUT" << Qt::endl;
