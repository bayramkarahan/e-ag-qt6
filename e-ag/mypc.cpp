...
void MyPc::timertcpConnectControlSlot()
{
    ...
    {
        if(connectState) setConnectState(false);
        setSshState("no");
        setVncState("no");
        setXrdpState("no");
        if(user!="noLogin") setUser("noLogin");
        tcpConnectCounter=0;
...
void MyPc::setUser(QString _usr)
{
    ...
    if(select&&connectState){
        qDebug()<<"setUser";
        emit pcClickSignal(mac,false);//nesneler arası data transferi***
    }
}
...
void MyPc::slotMouseClick(bool _select)
{
    QPalette palet;
    select=_select;
    //qDebug()<<"slotMouseClick"<<_select;
    if(_select){
        ...
        //emit pcClickSignal(mac,_multiSelect);//nesneler arası data transferi***
    }
    else{
        palet.setColor(QPalette::Window, QColor(0,0,0,0));
        setPalette(palet);
...
void MyPc::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button()==Qt::LeftButton)
    {
        if (event->modifiers() & Qt::ControlModifier) {
            ...
            {
               multiUnselectPc();
               slotMouseClick(false);
               emit pcClickSignal(mac,true);//nesneler arası data transferi***
               //qDebug()<<"çoklu seçimden çıkarılıyor";
            }else
            {
               slotMouseClick(true);
               multiSelectPc();
               emit pcClickSignal(mac,true);//nesneler arası data transferi***
               //qDebug()<<"çoklu seçim modunda";
            }
        }else
        {
             slotMouseClick(true);
             //qDebug()<<"tekli seçim modunda";
             emit pcClickSignal(mac,false);//nesneler arası data transferi***
        }
    }
}
...
void MyPc::mouseReleaseEvent(QMouseEvent *event)
{
 // QWidget::mouseReleaseEvent(event);
}

void MyPc::multiSelectPc(){
    //if(connectState)
   // {
        multiSelect=true;
...
QPixmap MyPc::svgToPixmap(const QString &svgPath, const QSize &size)
{
    QSize safeSize = size;
    if (safeSize.isEmpty()) {
        safeSize = QSize(64, 64); // fallback
    }

    QSvgRenderer r(svgPath);
    if (!r.isValid()) {
        qDebug() << "HATA: SVG yüklenemedi:" << svgPath;
        return QPixmap();
    }
    ...
    QPainter p(&pixmap);
    if (!p.isActive()) {
        qDebug() << "HATA: Painter aktif değil!";
        return QPixmap();
    }

    QRectF target(QPointF(0,0), safeSize);
