...
CpuVideoDecoder::CpuVideoDecoder(QObject *parent)
    : AbstractVideoDecoder(parent)
{
}
AbstractVideoDecoder::Type CpuVideoDecoder::type() const
{
    return Type::CPU;
}

CpuVideoDecoder::~CpuVideoDecoder()
{
    stop();
}
void CpuVideoDecoder::setSettings(const StreamSettings &settings)
{
    m_settings=settings;
}
bool CpuVideoDecoder::start()
{
    stop();
    ...
    qDebug() << "CPU video H264 decoder initialized.";

    return true;
}

void CpuVideoDecoder::stop()
{
    if(m_packet)
...
void CpuVideoDecoder::decode(const QByteArray &packet,
                         qint64 pts,
                         bool marker)
{
 /*   qDebug()
        << "CpuVideoDecoder VIDEO INPUT"
        << "size =" << packet.size()
        << "pts =" << pts
        << "marker =" << marker;*/
   ...
   /* qDebug()
    ...
    if(av_new_packet(m_packet, packet.size()) < 0)
        return;
