...
void AudioOpusDecoder::decode(const QByteArray &data,
                              qint64 pts)
{
    if (!m_decoder || data.isEmpty())
        return;

    const int maxSamples = 960;

    QByteArray pcm;
    pcm.resize(maxSamples * m_channels * sizeof(opus_int16));
    ...
    pcm.resize(samples * m_channels * sizeof(opus_int16));
